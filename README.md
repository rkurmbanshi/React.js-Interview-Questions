...
<details> <summary><strong>175. Create a Counter App and Implement These Functionalities (Start Counter, Pause Counter, Reset Counter)</strong></summary>
A simple Counter app in React with start, pause, and reset functionality:
...
function CounterApp() {
  ...
  const startCounter = () => setIsCounting(true);
  const pauseCounter = () => setIsCounting(false);
  const resetCounter = () => {
    setCount(0);
    setIsCounting(false);
  };
...
<details> <summary><strong>176. What is Selectors in CSS?</strong></summary>
CSS selectors are patterns used to select and style HTML elements based on attributes like element type, class, id, etc. There are different types of selectors: class, id, descendant, child, etc.
...
.button { color: red; } /* Selects all elements with class "button" */
#header { color: green; } /* Selects the element with id "header" */
</details>
<details> <summary><strong>177. What is Sass in CSS?</strong></summary>
Sass (Syntactically Awesome Stylesheets) is a CSS preprocessor that adds features like variables, nesting, and mixins to make CSS more maintainable and dynamic.
...
<details> <summary><strong>178. Explain Features of React JS</strong></summary>
React is a powerful JavaScript library for building user interfaces. Some key features of React include:

Virtual DOM: Enhances performance by updating only the changed parts of the DOM.
Reusable Components: React promotes the creation of small, reusable components.
One-way Data Binding: Data flows in one direction (from parent to child).
JSX: Allows HTML to be written in JavaScript, making it easier to design UI.
</details>
<details> <summary><strong>179. Explain Disadvantages of React JS</strong></summary>
Some disadvantages of React include:

Steep Learning Curve: React has a lot of features and concepts that may take time to master.
Fast-Paced Development: Frequent updates and changes can sometimes make it challenging to stay up-to-date.
Boilerplate Code: Setting up projects can require significant initial configuration.
</details>
<details> <summary><strong>180. How to Handle Pagination and List Scrolling in React JS If the Quantity of Data is More Than? (Explain React Window and React Virtualization)</strong></summary>
When dealing with large lists of data, you can use libraries like React Window or React Virtualization to improve performance by rendering only the visible items.
...
<details> <summary><strong>181. What is Forward Ref in React JS?</strong></summary>
Forward ref allows you to pass a ref from a parent component to a child component. This is useful when you need to interact with a DOM element inside a child component.
...
<details> <summary><strong>182. Explain setState is Synchronous or Asynchronous in useState Hook</strong></summary>
The setState function from the useState hook is asynchronous in React. This means that updates to the state do not immediately reflect, and React batches state updates for performance reasons.
...
const increment = () => {
  setCount(count + 1);
  console.log(count); // May log the old value because the state update is async.
};
</details>
<details> <summary><strong>183. Create Custom Hook for the API Call</strong></summary>
You can create a custom hook to handle API calls efficiently in React.
...
<details> <summary><strong>184. What is Accessibility?</strong></summary>
Accessibility (a11y) refers to the design and development of websites or applications in a way that ensures people with disabilities can use them effectively. It includes considerations such as keyboard navigation, screen reader compatibility, and color contrast.

</details>
<details> <summary><strong>185. What is Position in CSS?</strong></summary>
The position property in CSS is used to specify the positioning method for an element. It can take values such as static, relative, absolute, fixed, or sticky.
...
<details> <summary><strong>186. What is React Fiber?</strong></summary>
React Fiber is the new reconciliation algorithm in React. It improves the rendering process by breaking the work into chunks and allowing React to prioritize updates. This results in better performance and user experience, especially in complex applications.

</details>
<details> <summary><strong>187. Explain Inline Element or Block Element</strong></summary>
Inline Element: An inline element only takes up as much width as necessary and does not break the flow of content (e.g., <span>, <a>).
Block Element: A block element takes up the entire width of its container and starts on a new line (e.g., <div>, <p>).
Example:

html
Copy code
<span>Inline</span> <span>Element</span>
<div>Block Element</div>
</details>
<details> <summary><strong>188. Difference Between Normal Export and Default Export</strong></summary>
Normal Export: You can export multiple items in a module using normal export.
Default Export: A module can only have one default export. It is often used when you export a single function or object.
Example:
...
<details> <summary><strong>189. How to create Todo App and Implement These Functionalities: Add Todo, Complete Todo, Edit Todo, Delete Todo</strong></summary>
Example:
      ...
      <ul>
        {todos.map((todo, index) => (
          <li key={index}>
            ...
            />
            {todo.text}
            <button onClick={() => editTodo(index, prompt('Edit Todo', todo.text))}>Edit</button>
            <button onClick={() => deleteTodo(index)}>Delete</button>
          </li>
        ))}
      </ul>
...
<details> <summary><strong>190. Create Counter App and Implement These Functionalities: Start Counter, Pause Counter, Reset Counter</strong></summary>
Example:
...
<details> <summary><strong>191. What is Micro Front-End?</strong></summary>
Micro Front-End is an architectural style where a single web application is composed of multiple, smaller applications (front-end components) that can be developed, deployed, and maintained independently. It is similar to microservices but for front-end applications.

</details>
<details> <summary><strong>192. Why Do We Need React JS? Explain in Detail</strong></summary>
React JS is widely used for building dynamic, high-performance user interfaces. It offers a component-based architecture, efficient rendering with the Virtual DOM, and is widely supported with a strong ecosystem of tools and libraries. React allows developers to build scalable, maintainable, and performant web applications.
